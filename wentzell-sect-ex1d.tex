...
%
=& 4L |f|^2 +2 c |f|^2 \big(1 + \sin 4kL \sin 2 \eta \big) \\
= & 1 \\
\Rightarrow &
|f|^2 = \frac{1}{4L} \Big( 1 + \frac 1 2 C (1 + \sin 4kL \sin 2\eta ) \Big)^{-1}
\end{split}
\end{equation*}
...
\begin{equation*}
|f_n|^2 = 
\frac{1}{4L}\Big( 1 - \frac{4}{\pi^2C L n^2} + \mathcal{O}\big(\frac{1}{n^3}\big) \Big)
\end{equation*}
...
+\underbrace{\sum_{n=1}^{+\infty}\big(4 L  |f_n|^2 -1 \big) e^{-ik_nt}}_{\textrm{uniformly convergent}}
\Big) \\
& = e \sum_{n=1}^{+\infty}\big(|f_n|^2 -\frac{1}{4L} \big)
\end{split}
\end{equation*}
...
\omega(\phi^2(x)\phi_2^\dagger(y)) =
 \sum_{k_n>0} |f_n|^2 e^{-ik_n(t-i\varepsilon)}
\end{equation*}
