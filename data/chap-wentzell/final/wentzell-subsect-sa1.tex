...
- p^j p_j = k^2
\end{equation*}
spans the solution space of the first equation of~\cref{wen-motion}. 
...
Considering the second equation of~\cref{wen-motion} and the condition that an element of $\dom(\Delta)$ should satisfy, one gets
...
we can find eigenvectors of $\Delta$ with eigenvalue $k$.
However, it should be noted that these plane wave solutions are only normalizable to $\delta$-function.
\end{proof}