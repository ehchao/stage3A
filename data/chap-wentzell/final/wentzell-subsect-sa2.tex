...
(p_d ^2 + c^{-2})(A^2 - B^2 )= 0
\end{equation*}
One can easily verify that $p_d = i \pm c^{-1}$ can not be solution of the equation. 
...
We show that
\begin{equation*}
\tan \theta = \frac{\mp\theta + a}{\theta \pm a} \quad a>0
\end{equation*}
does not have solution $\theta \in \mathbb{C} - \mathbb{R}$ since the equation implies
\begin{equation*}
\Im (\tan\theta) = \frac{\Im(a\theta^*)}{|\theta \pm a|^2}
\end{equation*}
which is impossible because $\Im\tan(x+iy)$ and $y$ are of the same sign for $x,y\in\mathbb{R}$.
%%
