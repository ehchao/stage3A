...
\begin{equation*}
\dom( \Delta) = \{ \Phi = (\phi, \phi_|) \in \mathcal{H} \enskip | \enskip \mathcal{P}_+\phi \vert_{\partial M} - \phi_| = 0 \}
\end{equation*}  
...
the higher time-derivatives of $\Phi$ can also be bounded by the Cauchy data at instant $0$ living in an appropriate Hilbert space.\\\\
...
Using Stokes' theorem, the integral of this quantity over 
%TODO add picture
gives
\begin{equation*}
\begin{split}
...
\Phi_0\in \cap_{k\in\mathbb{N}}\overline{\mathcal{H}^k_{\varepsilon_n}(M) }
\end{equation}
