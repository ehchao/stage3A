...
%Consider now a system confined in $[-\frac{L}{2}, \frac{L}{2}]$. 
%We apply the procedure given in~\cite{Zahn2015} in order to calculate the vacuum polarisation. 
Suppose that the the solution takes the form $\phi = \begin{pmatrix} \phi_L \\ \phi_R \end{pmatrix}$.
In terms of $\phi = \gamma^0 \psi$, the boundary condition~\cref{vacuum-bagboundcond} becomes
\begin{equation*}
\begin{pmatrix}
-i \phi_R \\
i \phi_L
...
According to \cref{nef-matching2}, the components of the solution in the region $x > 0$ should be $\phi_L = \frac{1}{D} (Af+Cg) e^{-ik(x^0 - x^1)}$ and $\phi_R = \frac{1}{D}(C^* f + Ag ) e^{-ik(x^0 + x^1)}$. 
...
%\footnote{The summation of Einstein is not applied on $A$ here.}
...
%\end{split}
%\end{equation}
We subtract then from the above the Hadamard parametrix~\cref{vacuum-hadamardparametrix} and trace them with the gamma matrices in order to get the components of the vacuum charge and current densities.
