...
\end{theorem}
Finally, we will use the generalized version of the Poincaré-Wirtinger inequalities
\begin{theorem}
Denote $\Omega^k_{q,p}(M) \equiv \{ \omega \in L^q(M, \Lambda^k) \vert \dd \omega \in L^p \}$ and $H^k_{q,p}$ the corresponding cohomology. Then
\begin{enumerate}
\item $H^k_{q,p}(M,g) = 0$ if and only if there exists a constant $C\leq +\infty$ such that for any closed $p$-integrable differential form $\omega$ of degree $k$, there exists a differential form $\theta$ of degree $k-1$ such that $\dd \theta = \omega$ and
\begin{equation}
\| \theta \| \leq C \| \omega \|_{L^p}
\end{equation}
%
\item
